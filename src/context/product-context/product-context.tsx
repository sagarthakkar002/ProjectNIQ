...
export const ProductProvider = ({ children, selectedCategory }: {children: ReactNode, selectedCategory: string}) => {
    ...
    const {response: productsRes} = useFetch(productsByCategoryUrl(selectedCategory), null, [selectedCategory]);
    const {response: productDetails} = useFetch(productDetailsUrl(selectProduct), null, [selectProduct]);

    const products = (!selectedCategory ? [] : productsRes as Product[] || []).map((product, index) => ({...product, rating: index + 10}))
